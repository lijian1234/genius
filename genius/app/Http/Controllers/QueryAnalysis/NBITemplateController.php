...
class NBITemplateController extends Controller
{
	...
	public function searchNBITreeData() {
        ...
        foreach ($users as $user) {
            $userStr = $user->user;
            $sql = "select * from templateNbi where user = '$userStr' and templateName like '$inputData'";
	...
	public function getElementTree(){
		...
		$dsn = "mysql:host=localhost;dbname=mongs";
        $db = new PDO($dsn, 'root', 'mongs');
        $sql = "select elementId from templateNbi where templateName = '$templateName' and user = '$user'";
		$resTem = $db -> query($sql);
		if($resTem){
	...
	public function updateFormula(){
        ...
        if($id){
        	if($user == "admin"){
        		$sql = "update kpiFormulaNbi set kpiName = '$name', kpiformula = '$formula', kpiPrecision = '$precision' where id = '$id'";
        	}else{
        		$sql = "update kpiFormulaNbi set kpiName = '$name', kpiformula = '$formula', kpiPrecision = '$precision' where id = '$id' and user = '$user'";
        	}
        	
        	$res = $db -> exec($sql);
        }else{
        	$sql = "insert into kpiFormulaNbi values (null,'$name','$user','$formula','$precision')";
        	$res = $db -> exec($sql);
        }
        if($res){
    		echo true;
	...
	public function deleteFormula(){
        ...
        $user = Auth::user() -> user;
        if($user =="admin"){
        	$sql = "delete from kpiFormulaNbi where id ='$id'";
        }else{
        	$sql = "delete from kpiFormulaNbi where id ='$id' and user = '$user'";
        }
	...
	public function updateElement(){
        ...
        $user = Auth::user() -> user;
        if($user == "admin"){
        	$sql = "UPDATE templateNbi SET elementId = '$ids' where id = '$id'";
        }else{
        	$sql = "UPDATE templateNbi SET elementId = '$ids' where id = '$id' and user = '$user'";
        }
		$res = $db -> exec($sql);
		if($res){
			echo true;
	...
	public function addMode(){
        ...
        $db = new PDO($dsn, 'root', 'mongs');
     	...
     	$sql = "insert into templateNbi values (null,'$templateName',null,'$description','$user')";
     	$res = $db -> query($sql);
		if($res){
			echo true;
		}else{
	...
	public function deleteMode(){
        ...
        if($user == "admin"){
        	$sql = "delete from templateNbi where id = '$id'";
        	$res = $db -> exec($sql);
        	if(!$res){
				echo "1";
			}else{
        	...
        	$sql = "delete from templateNbi where id = '$id' and user = '$user'";
        	$res = $db -> exec($sql);
        	if($res){
				echo "1";
			}else{
	...
	public function copyMode(){
        ...
        $sql = "select elementId from templateNbi where id = '$id'";
        $res = $db -> query($sql);
        $item = $res->fetchAll(PDO::FETCH_ASSOC);
        $elementId = $item[0]['elementId'];
        $user = Auth::user() -> user;
     	$sql = "insert into templateNbi values (null,'$templateName','$elementId','$description','$user')";
     	$res = $db -> exec($sql);
		if($res){
			echo true;
		}else{
