...
class LTETemplateController extends Controller
{
	...
	public function searchLTETreeData() {
        ...
        foreach ($users as $user) {
            $userStr = $user->user;
            $sql = "select * from template where user = '$userStr' and templateName like '$inputData'";
	...
	public function getElementTree(){
		...
		$dsn = "mysql:host=localhost;dbname=mongs";
        $db = new PDO($dsn, 'root', 'mongs');
        $sql = "select elementId from template where templateName = '$templateName' and user = '$user'";
        //echo($sql); return;
		$resTem = $db -> query($sql);
		if($resTem){
	...
	public function getTreeTemplate(){
		...
		$rsUsers =$db -> query("select distinct user from $table"); //筛选所有user
		$idNum = 1; 
		$items = array(); //存储最终结果
	...
	public function updateFormula(){
        ...
        if($id){
        	if($user == "admin"){
        		$sql = "update kpiformula set kpiName = '$name', kpiformula = '$formula', kpiPrecision = '$precision' where id = '$id'";
        	}else{
        		$sql = "update kpiformula set kpiName = '$name', kpiformula = '$formula', kpiPrecision = '$precision' where id = '$id' and user = '$user'";
        	}
        	
        	$res = $db -> exec($sql);
        }else{
        	
        	$sql = "insert into kpiformula values (null,'$name','$user','$formula','$precision')";
        	$res = $db -> exec($sql);
        }
        if($res){
    		echo true;
	...
	public function deleteFormula(){
        ...
        $user = Auth::user() -> user;
        if($user == "admin"){
        	$sql = "delete from kpiformula where id ='$id'";
        }else{
        	$sql = "delete from kpiformula where id ='$id' and user = '$user'";
        }
	...
	public function updateElement(){
        ...
        $db = new PDO($dsn, 'root', 'mongs');
     	...
     	$user = Auth::user() -> user;
     	if($user == "admin"){
     		$sql = "UPDATE template SET elementId = '$ids' where id = '$id'";
     	}else{
     		$sql = "UPDATE template SET elementId = '$ids' where id = '$id' and user = '$user'";
     	}
	...
	public function addMode(){
        ...
        $db = new PDO($dsn, 'root', 'mongs');
     	...
     	$sql = "insert into template values (null,'$templateName',null,'$description','$user')";
     	$res = $db -> query($sql);
		if($res){
			echo true;
		}else{
	...
	public function deleteMode(){
        ...
        if($user == "admin"){
        	...
        	$sql = "delete from template where id = '$id' and user = '$user'";
        	$res = $db -> exec($sql);
        	if($res){
				echo "1";
			}else{
	...
	public function copyMode(){
        ...
        $sql = "select elementId from template where id = '$id'";
        $res = $db -> query($sql);
        $item = $res->fetchAll(PDO::FETCH_ASSOC);
        $elementId = $item[0]['elementId'];
        $user = Auth::user() -> user;
     	$sql = "insert into template values (null,'$templateName','$elementId','$description','$user')";
     	$res = $db -> exec($sql);
		if($res){
			echo true;
		}else{
