...
class currentAlarmQueryController extends Controller{
	...
	public function getTableData(){
		$dsn = "mysql:host=localhost;dbname=Alarm";
                ...
                if($placeDimName !=''){
                        $queryFilter = " where city in (".$citystr.") and ".$placeDim."='$placeDimName' and Perceived_severity != 5 order by Event_time desc";
                }else{
                        $queryFilter = " where city in (".$citystr.") and  Perceived_severity != 5 order by Event_time desc";
                }

                $result = array();
                //$rs = mysql_query("select count(*) from ".$_REQUEST['table'].$queryFilter);
                $rs = $db->query("select count(*) from FMA_alarm_list".$queryFilter);
                $row = $rs->fetchAll(PDO::FETCH_ASSOC);
                $result["total"] = $row[0]['count(*)'];

                $sql = "select Event_time,city,subNetwork,meContext,eutranCell,SP_text,Problem_text,Alarm_id from FMA_alarm_list".$queryFilter.$limit;
                $res = $db->query($sql);
                $row = $res->fetchAll(PDO::FETCH_ASSOC);
                $items = array();
                foreach ($row as $qr) { 
        ...
        public function getAllTableData(){
                ...
                $result = array();
                $sql = "select Event_time,city,subNetwork,meContext,eutranCell,SP_text,Problem_text,Alarm_id from FMA_alarm_list".$queryFilter;
                ...
                $result["text"] = "Event_time,city,subNetwork,meContext,eutranCell,SP_text,Problem_text,Alarm_id";
                $result['rows'] = $items;
