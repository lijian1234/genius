...
class relationBadHandoverController extends Controller{
    ...
    public function getDataHeader(){
        ...
        $head = $db ->query("select COLUMN_NAME from information_schema.COLUMNS where table_name = 'RelationBadHandover' and table_schema = 'AutoKPI'");
        $text = array();
        foreach ($head as $h) {
	...
	public function getTableData(){
        ...
        $result = array();
        $rs = $db->query("select count(*) from RelationBadHandover where city in ($citystr)");
        $row = $rs->fetchAll(PDO::FETCH_ASSOC);
        $result["total"] = $row[0]['count(*)'];

        $sql = "select * from RelationBadHandover where city in ($citystr)".$limit;
        $res = $db->query($sql);
        $row = $res->fetchAll(PDO::FETCH_ASSOC);
        $items = array();
        foreach ($row as $qr) { 
    ...
    public function getAllTableData(){
        ...
        $sql = "select * from RelationBadHandover where city in ($citystr)";
        $res = $db->query($sql);
        $row = $res->fetchAll(PDO::FETCH_ASSOC);
        $items = array();
        foreach ($row as $qr) { 
        ...
        $filename="common/files/RelationBadHandover_".$phpCity."_".date('YmdHis').".csv";
