...
function setElementTree(text,user){
	var tree = '#LTEElementTree';
	$(tree).treeview({
		data: getElementTree(text,user),
	  	onNodeSelected: function(event, data) {
	      	findTemplateById(data.id);
	   	}
	}); //树
}
function getElementTree(text,user) {
...
function setFormulaTreeTable(){
	...
	$(table).treegrid({    
	    ...
	    treeField:'kpiName',    
	    columns:[[    
	        {title:'Name',field:'kpiName',width:140},    
	        {field:'kpiFormula',title:'Formula',width:270},    
	        {field:'kpiPrecision',title:'Precision',width:80}
	    ]],
